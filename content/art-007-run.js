...
runTest(1, 'Representative recipe validates without unknown fields or reserved dependencies', () => {
  ...
  assert(Array.isArray(recipe.ingredientGroups), 'ingredientGroups must be an array')
  assert(Array.isArray(recipe.instructionSections), 'instructionSections must be an array')
})
...
runTest(6, 'The canonical content directory is isolated from build output — projections cannot write back', () => {
  const websiteDir = path.join(__dirname, '..', 'sunday-brunch-website')
  const viteCfgPath = path.join(websiteDir, 'vite.config.js')
  const viteCfg = fs.readFileSync(viteCfgPath, 'utf-8')

  // Determine the build output directory. Distinguish an explicit outDir from the Vite default.
  const outDirMatch = viteCfg.match(/outDir\s*:\s*['"]([^'"]+)['"]/)
  const explicitOutDir = outDirMatch ? outDirMatch[1] : null
  const effectiveOutDir = explicitOutDir !== null ? explicitOutDir : 'dist'

  console.log(`Checked:  sunday-brunch-website/vite.config.js`)
  console.log(`Build outDir: ${explicitOutDir !== null ? `'${explicitOutDir}' (explicit)` : 'not set (Vite default: dist)'}`)

  // The build output must resolve to a location OUTSIDE the canonical content/ directory,
  // so no projection can ever overwrite canonical source. Resolve absolutely to catch
  // traversal cases like '../content/feeds'.
  const resolvedOut = path.resolve(websiteDir, effectiveOutDir)
  const resolvedContent = path.resolve(ROOT)
  console.log(`Resolved build output: ${resolvedOut}`)
  console.log(`Canonical content dir: ${resolvedContent}`)
  assert(resolvedOut !== resolvedContent && !resolvedOut.startsWith(resolvedContent + path.sep),
    `Build output '${resolvedOut}' must not fall inside canonical content '${resolvedContent}'`)

  // content/ directory must contain no build artefacts.
  ...
  console.log(`Canonical content/ is a read-only source. Build output goes to 'sunday-brunch-website/${effectiveOutDir}'.`)
